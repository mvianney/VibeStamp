...
function CustomerMain({
  ...
  // Sub-screens: 'loyalty' | 'scanner' | 'pay' | 'receipt' | 'loyalty-detail'
  const [activeTab, setActiveTab] = useState<'loyalty' | 'scanner' | 'pay' | 'receipt' | 'loyalty-detail'>('loyalty');
  ...
  const loadCustomerData = async () => {
    try {
      ...
      const list = await getCustomerLoyaltyCards(profile.walletPublicKey);
      
      const mStates: Record<string, MerchantState> = {};
      for (const card of list) {
        if (!mStates[card.merchant]) {
          const state = await getMerchantState(card.merchant);
          if (state) {
  ...
  const handleFundWallet = async () => {
    ...
    try {
      ...
      console.error(e);
      alert(`Faucet request failed: ${e.message || e}`);
    } finally {
      setFunding(false);
    }
  };
  ...
  const handleConfirmPay = async () => {
    ...
    try {
      ...
      setTxSignature(sig);
      setPayStatusText('Recording on-chain reward states...');

      // Record purchase in simulated database
      const lamports = Math.round(scannedUriData.amount * LAMPORTS_PER_SOL);
      const { card, pointsEarned, streakBonus, tierBonus } = await recordPurchase(
        scannedUriData.recipient,
        profile.walletPublicKey,
        lamports,
        sig
      );
  ...
  const handleRedeemPoints = async () => {
    ...
    try {
      const updatedCard = await redeemPoints(selectedCard.merchant, profile.walletPublicKey, pointsToRedeem);
      const discountUSD = pointsToRedeem / rate;
      setRedeemStatus({
        success: true,
        msg: `Claimed! Successfully redeemed ${pointsToRedeem.toLocaleString()} STAMP for a $${discountUSD.toFixed(2)} discount reward!`
      });
    ...
    <div className="app-shell">
      ...
      <div className="profile-banner-wrap" style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '20px', background: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: '16px', padding: '16px 24px', marginBottom: '24px', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          >
            {funding ? <RefreshCw className="animate-spin-slow" size={14} /> : <Wallet size={14} />}
            <span style={{ marginLeft: '6px' }}>{funding ? 'Funding...' : 'Fund Wallet'}</span>
          </button>
        </div>
      </div>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <button 
                ...
                style={{ padding: '12px 20px', borderRadius: '30px', display: 'flex', alignItems: 'center', gap: '8px' }}
              >
                <Camera size={16} /> Scan Merchant QR Code
              </button>
            </div>
            ...
            {loadingCards ? (
              ...
              <div className="empty-state" style={{ padding: '60px 24px', background: 'var(--bg-surface)', border: '1px dashed var(--border-color)', borderRadius: '24px', textAlign: 'center' }}>
                ...
                <p style={{ maxWidth: '320px', margin: '8px auto 20px auto', fontSize: '13px', color: 'var(--text-secondary)' }}>
                ...
                <button 
                  ...
                  onClick={() => setActiveTab('scanner')}
                >
                  <Camera size={16} style={{ marginRight: '6px' }} /> Open Scanner
                </button>
              </div>
            ) : (
        ...
        {activeTab === 'scanner' && (
          <div style={{ maxWidth: '500px', margin: '0 auto', textAlign: 'center' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              ...
              <h2 style={{ fontSize: '18px', fontWeight: 700 }}>Scan Payment QR</h2>
              <div style={{ width: 80 }} /> {/* Spacer */}
            </div>

            <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '16px' }}>
              Align the merchant's checkout QR code inside the target boundary to pay and claim stamps automatically.
            </p>

            {cameraError ? (
              <div className="panel" style={{ padding: '24px', border: '1px solid rgba(239, 68, 68, 0.2)', background: 'rgba(239, 68, 68, 0.02)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
                <AlertTriangle size={32} style={{ color: 'var(--color-secondary)' }} />
                <h3>Camera Permission Required</h3>
                <p style={{ fontSize: '12.5px', color: 'var(--text-secondary)' }}>{cameraError}</p>
                <button className="btn btn-secondary" onClick={() => setActiveTab('loyalty')}>
                  Go Back
                </button>
              </div>
            ) : (
